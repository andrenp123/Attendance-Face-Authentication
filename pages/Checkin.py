...
st.set_page_config(
    page_title="Our New Attendance System",
    page_icon="👋",
)
...
def attendance():
    ...
    variable=[username,'Checkin']
    mycursor.callproc('spInsertAbsensi',variable)
    connection.commit()
    mycursor.close()
    connection.close()
...
def countcosine(testface, trainingface):
    transposeimg=np.transpose(trainingface)
    x = np.sum(np.multiply(trainingface, trainingface))
    y = np.sum(np.multiply(testface, testface))
    z = np.matmul(transposeimg, testface)
    cosine = 1 - (z / (np.sqrt(x) * np.sqrt(y)))
    return cosine

def normalizel2(n, axis=-1, epsilon=1e-10):
    y = np.sqrt(np.maximum(np.sum(np.square(n), axis=axis, keepdims=True), epsilon))
    score = n/y
    return score
    
def counteuclidean(testface, trainingface):
    x = trainingface - testface
    euclidean = np.sum(np.multiply(x, x))
...
if authentication_status==True and username=='controller':
    st.warning("Controller can't access Checkin Menu")
        
elif authentication_status:
    datatraining=[]
    dir_training="./database/%s" %(username)

    for file in listdir(dir_training): 
        data, extension = file.split(".")
        training = './database/%s/%s.jpg' %(username,data)
        datatraining.append(model.predict(preprocess_image(training))[0,:])

    st.write('Hello ,  *%s*' %(name))
    st.caption('Welcome to Face Authentication System')
    label=""
    picture = st.camera_input(label,label_visibility="hidden")
    if picture:
        datatest = model.predict(preprocess_image(picture))[0,:]
        cosine=0
        for x in datatraining:
            cosine=cosine+countcosine(datatest,x)
        cosine=cosine/len(datatraining)
        if cosine<0.025:
            #st.session_state.authentication_status = None
            attendance()
            st.subheader('Face Verified :white_check_mark: Absensi anda berhasil')
        else:
            st.write('Face Not Verified :x:')
